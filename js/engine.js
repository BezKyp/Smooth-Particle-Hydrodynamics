let Engine = (function() {
  ...
  function Grid() {
    ...
    this.init = function(nx, ny, w, h) {
      console.log("Grid.init", nx, ny, w, h);
      this.nx = nx;
      this.ny = ny;
      this.w = w;
      this.h = h;
  ...
  return {
    ...
    setNumParticles: function(n) {
      ...
      if (particles == undefined) {
        particles = new Array(n + 1);
      }
      else {
        if (n < particles.length) {
          grid.hardreset();
          particles.length = n + 1;
        }
        i0 = particles.length;
      }
    ...
    forceVelocity: function(x, y, Vx, Vy) {
      ...
      forceVx = Vx;
      forceVy = -Vy;
      particles[particles.length - 1].x = x / domainScale;
      particles[particles.length - 1].y = ymax - y / domainScale;
      particles[particles.length - 1].Vx = Vx;
      particles[particles.length - 1].Vy = -Vy;
    },
